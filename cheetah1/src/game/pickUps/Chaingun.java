 ...
 * @since 2019
 */
public class Chaingun extends GameComponent {

    public final float 			PICKUP_THRESHHOLD = 0.75f;
    private static final String WEAPONS_RES_LOC = "weapons/";
    private static final String RES_LOC = "chaingun/P_CGUN";
     ...
     */
    public Chaingun(Transform transform, boolean shouldFloat) {
        if (mesh == null) {
            float sizeY = 0.25f;
            //									TODO: this equals 4 divided by "siseY" times the width of the texture
            float sizeX = (float) ((double) sizeY / (0.1785714286f * 4.0));
        ...
        if (material == null) {
            material = new Material(new Texture(WEAPONS_RES_LOC + "chaingun/UMGPA0"));
        }
	...
	public void update(double delta) {
        ...
        if (distance < PICKUP_THRESHHOLD && Level.getPlayer().isChaingun() == false) {
        	AudioUtil.playAudio(PICKUP_NOISE, 0);
            Level.getPlayer().setChaingun(true);
            Level.removeChainGun(this);
        }
    }
